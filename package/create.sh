...
cp -ar ../fluffychat/build/linux/x64/release/bundle/* "$PROGDIR/"
cp -ar usr "$PACKDIR/"
mkdir -p "$PACKDIR/usr/share/icons/hicolor/scalable/apps"
cp -ar ../fluffychat/assets/logo.svg "$PACKDIR/usr/share/icons/hicolor/scalable/apps/im.fluffychat.svg"
...
PACKAGE_VERSION="$(grep 'version: ' ../fluffychat/pubspec.yaml | sed 's/version: \(.*\)+[0-9]\+/\1/')"
